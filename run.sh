...
if [ -f "$PPATH/$PKEY" ]; then
  echo "$PPATH/$PKEY will be used for SSH connection"
else
...
echo "3.1) If you are using private key for ssh connection, key folder needs to be mapped to /key, e.g. -v /key-folder:/key"
